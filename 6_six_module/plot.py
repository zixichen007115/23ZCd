...
img_dir = "../0_files/data_six/img_" + config.task
...
for step in range(0, ctrl_step, 5):
    ...
    ax.set_zlim([0.0, 0.3])
    ax.set_xticks(np.arange(-0.15, 0.25, 0.1))
    ax.set_yticks(np.arange(-0.15, 0.25, 0.1))
    ax.set_zticks(np.arange(0.0, 0.40, 0.1))
    label_font = 15
...
plt.savefig("../0_files/data_six/" + config.task + "_diagram.png")
plt.close()
...
imageio.mimsave("../0_files/data_six/" + config.task, frames, 'GIF', duration=0.1)
