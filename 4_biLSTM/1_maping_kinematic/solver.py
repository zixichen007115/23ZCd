...
class Solver(object):
    ...
    def build_model(self, hidden_size=128, num_layers=4, output_size=2):
        """Create a classifier."""
        input_size = self.t_step * 5 + 1
        self.LSTM = LSTM(input_size, hidden_size, num_layers, output_size, self.device)
    ...
    def test(self):
        ...
        # calculate test acc
        with torch.no_grad():
            err = np.zeros([len(self.loader_test.dataset), self.num_seg, 2])
            num = 0
