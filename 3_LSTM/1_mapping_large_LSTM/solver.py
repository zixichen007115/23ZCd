...
class Solver(object):
    ...
    def build_model(self, input_size=20, hidden_size=128, num_layers=4, num_classes=8):
        """Create a classifier."""
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            if (i + 1) % self.early_stop_step == 0:
                ...
                err = 0
                with torch.no_grad():
                    for i, (seg_input, output) in enumerate(val_data_iter):
                        seg_input = seg_input.to(self.device)
                        est_output = self.LSTM(seg_input)
    ...
    def test(self):
        ...
        # calculate test acc
        with torch.no_grad():
            err_act = np.zeros([len(self.loader_test.dataset), 5, 8])
            num = 0
            for i, (pos, act) in enumerate(test_data_loader):
