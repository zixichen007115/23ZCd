...
def main(config):
    ...
    if config.mode == 'train':
        loader_train = get_loader(config.batch_size, 'train', int(config.model_name))
        loader_test  = get_loader(config.batch_size, 'val', int(config.model_name))

    else:
        loader_train = []
        loader_test  = get_loader(config.batch_size, 'test', int(config.model_name))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default="0", help='the name of model')
